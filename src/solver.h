...
//methods
	...
	const VOL_RES_CLS 	call_vinci() { 
							if (!islra()) err_logic_vinci();
							return volume_computation(bsols.back()) * multiplier.back(); };
	const VOL_RES_CLS 	call_vinci(const unsigned int index) {
							if (!islra()) err_logic_vinci();
							return volume_computation(bsols[index]) * multiplier[index]; };
	const VOL_RES_CLS 	call_polyvest(const double epsilon, const double delta, const double coef) {
							if (!islra()) err_logic_polyvest();
							return volume_estimation(bsols.back(), epsilon, delta, coef) * multiplier.back(); };
	const VOL_RES_CLS 	call_polyvest(const unsigned int index, const double epsilon, const double delta, const double coef) {
							if (!islra()) err_logic_polyvest();
...
//methods
	...
	const VOL_RES_CLS 	volume_computation_light(int *bools, int var);
	const VOL_RES_CLS 	volume_computation_basic(int *bools, unsigned int nRows, std::vector<int> vars);
	const VOL_RES_CLS 	volume_computation(int *boolsol);
