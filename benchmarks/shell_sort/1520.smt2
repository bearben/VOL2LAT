(set-logic QF_LIA)
;1520)
;time-consuming:0.18
(set-info :status sat)
(declare-fun symbol_1 () Int)
(declare-fun symbol_0 () Int)
(declare-fun symbol_3 () Int)
(declare-fun symbol_2 () Int)
(declare-fun symbol_5 () Int)
(declare-fun symbol_4 () Int)
(declare-fun symbol_6 () Int)
(declare-fun symbol_7 () Int)
(declare-fun symbol_9 () Int)
(declare-fun symbol_8 () Int)
(declare-fun symbol_11 () Int)
(declare-fun symbol_10 () Int)
(declare-fun symbol_12 () Int)
(declare-fun symbol_13 () Int)
(declare-fun symbol_14 () Int)
(declare-fun symbol_15 () Int)
(declare-fun symbol_16 () Int)
(declare-fun symbol_17 () Int)
(declare-fun symbol_18 () Int)
(declare-fun symbol_19 () Int)
(declare-fun symbol_20 () Int)
(declare-fun symbol_21 () Int)
(declare-fun symbol_22 () Int)
(declare-fun symbol_23 () Int)
(assert
 (< (+ (+ (+ (+ symbol_0 1) 1) 1) 1) symbol_1))
(assert
 (not (> (- symbol_2 symbol_3) 0)))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (let ((?x2647 (+ ?x2773 1)))
 (let ((?x5588 (+ ?x2647 1)))
 (>= ?x5588 symbol_0)))))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (let ((?x2647 (+ ?x2773 1)))
 (let ((?x5588 (+ ?x2647 1)))
 (< ?x5588 symbol_1)))))
(assert
 (not (> (- symbol_4 symbol_5) 0)))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (let ((?x2647 (+ ?x2773 1)))
 (let ((?x6918 (- ?x2647 symbol_0)))
 (>= (- ?x6918 symbol_0) symbol_0)))))
(assert
 (> (- symbol_6 symbol_5) 0))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (let ((?x2647 (+ ?x2773 1)))
 (let ((?x6918 (- ?x2647 symbol_0)))
 (>= ?x6918 symbol_0)))))
(assert
 (> (- symbol_7 symbol_5) 0))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (let ((?x2647 (+ ?x2773 1)))
 (>= ?x2647 symbol_0))))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (let ((?x2647 (+ ?x2773 1)))
 (< ?x2647 symbol_1))))
(assert
 (not (> (- symbol_8 symbol_9) 0)))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (>= ?x2773 symbol_0)))
(assert
 (let ((?x2773 (+ symbol_0 1)))
 (< ?x2773 symbol_1)))
(assert
 (not (> (- symbol_10 symbol_11) 0)))
(assert
 (let ((?x7218 (- symbol_0 symbol_0)))
 (let ((?x4331 (- ?x7218 symbol_0)))
 (let ((?x7321 (- ?x4331 symbol_0)))
 (let ((?x6981 (- ?x7321 symbol_0)))
 (>= (- ?x6981 symbol_0) symbol_0))))))
(assert
 (> (- symbol_12 symbol_11) 0))
(assert
 (let ((?x7218 (- symbol_0 symbol_0)))
 (let ((?x4331 (- ?x7218 symbol_0)))
 (let ((?x7321 (- ?x4331 symbol_0)))
 (let ((?x6981 (- ?x7321 symbol_0)))
 (>= ?x6981 symbol_0))))))
(assert
 (> (- symbol_13 symbol_11) 0))
(assert
 (let ((?x7218 (- symbol_0 symbol_0)))
 (let ((?x4331 (- ?x7218 symbol_0)))
 (let ((?x7321 (- ?x4331 symbol_0)))
 (>= ?x7321 symbol_0)))))
(assert
 (> (- symbol_14 symbol_11) 0))
(assert
 (let ((?x7218 (- symbol_0 symbol_0)))
 (let ((?x4331 (- ?x7218 symbol_0)))
 (>= ?x4331 symbol_0))))
(assert
 (> (- symbol_15 symbol_11) 0))
(assert
 (let ((?x7218 (- symbol_0 symbol_0)))
 (>= ?x7218 symbol_0)))
(assert
 (> (- symbol_16 symbol_11) 0))
(assert
 (>= symbol_0 symbol_0))
(assert
 (< symbol_0 symbol_1))
(assert
 (not (> (* 2 symbol_0) symbol_1)))
(assert
 (let ((?x2884 symbol_1))
 (> (* 2 symbol_17) ?x2884)))
(assert
 (let ((?x2884 symbol_1))
 (> (* 2 symbol_18) ?x2884)))
(assert
 (let ((?x2884 symbol_1))
 (> (* 2 symbol_19) ?x2884)))
(assert
 (let ((?x2884 symbol_1))
 (> (* 2 symbol_20) ?x2884)))
(assert
 (let ((?x2884 symbol_1))
 (> (* 2 symbol_21) ?x2884)))
(assert
 (let ((?x2884 symbol_1))
 (> (* 2 symbol_22) ?x2884)))
(assert
 (let ((?x2884 symbol_1))
 (> (* 2 symbol_23) ?x2884)))
(assert
 (not (<= symbol_1 1)))
(check-sat)
